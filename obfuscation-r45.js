...
(function () {
  ...
  function buildSnippetBody(whitelistedHost) {
    ...
    return [
      ...
      "var blockedNodes = temporaryDiv.querySelectorAll('script,style');",
      "for (var b = 0; b < blockedNodes.length; b += 1) { if (blockedNodes[b] && blockedNodes[b].parentNode) { blockedNodes[b].parentNode.removeChild(blockedNodes[b]); } }",
      "var selectedText = selection.toString();",
      "if (!selectedText) { selectedText = temporaryDiv.innerText || temporaryDiv.textContent || ''; }",
      "selectedText = selectedText.replace(/\\u00a0/g, ' ').replace(/\\s*\\n\\s*/g, ' ').replace(/[ \\t]{2,}/g, ' ').trim();",
      ...
      "handled = true;",
      "}",
      "if (!handled && window.isSecureContext && window.navigator && window.navigator.clipboard && window.navigator.clipboard.writeText) {",
      "window.navigator.clipboard.writeText(plainTextData).catch(function() {});",
      "handled = true;",
      "}",
      "if (handled && event && event.preventDefault) { event.preventDefault(); }",
